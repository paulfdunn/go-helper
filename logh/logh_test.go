...
func TestDefaultOutput(t *testing.T) {
	...
	defaultOutput = wf
	testSetup(t)
	err = New(loggerName, "", DefaultLevels, Debug, 0, 0, 0)
	if err != nil {
	...
	out := string(buf[0:n])
	fmt.Printf("%s", out)
	// Note that this string is padded with extra length assuming default levels.
